...
public class Database {
	...
	public void insertLocation(String name, String address, int rent) {
		...
		Query query = session.createNativeQuery(
				"INSERT INTO locations  (name, address, rent) VALUES (:newName, :newAddress, :newRent) ");
	...
	public void insertPlayer(String playerName, String generatedPassword, String status, boolean value) {
		...
		Query query = session.createNativeQuery(
				"INSERT INTO players  (username, password, status, password_flag) VALUES (:newName, :newPassword, :newStatus, :newPasswordFlag) ");
	...
	public void insertContest(Player player_1, Player player_2, Location location, Date date, String result,
			Player winner) {
		...
		Query query = session.createNativeQuery(
				"INSERT INTO contests  (player_1, player_2, location_id, date, result, winner) VALUES (:newPlayer_1, :newPlayer_2, :newLocation_id, :newDate, :newResult, :newWinner ) ");
		query.setParameter("newPlayer_1", player_1);
		query.setParameter("newPlayer_2", player_2);
		query.setParameter("newLocation_id", location);
	...
	public List<Player> getPlayersByEntry(String playerName, String playerPassword) {
		...
		Query query = session.createNativeQuery(
				"SELECT * FROM players WHERE username=:newName AND password=:newPassword", Player.class);
		query.setParameter("newName", playerName);
	...
	public void updatePlayerPassword(String playerName, String newPassword, boolean value, String pPassword) {

		Session session = sessionFactory.openSession();
		session.beginTransaction();

		Query query = session.createNativeQuery("UPDATE players SET password=:newPassword, password_flag=:newValue "
				+ "WHERE username=:newName AND password=:pPassword", Player.class);
		query.setParameter("newName", playerName);
		query.setParameter("newPassword", newPassword);
		query.setParameter("newValue", value);
		query.setParameter("pPassword", pPassword);
	...
	public List<Contest> getContestsByPlayer(Player player) {
		...
		Query query = session.createNativeQuery(
				"SELECT * FROM contests WHERE player_1=:newPlayer1 OR player_2=:newPlayer2", Contest.class);
